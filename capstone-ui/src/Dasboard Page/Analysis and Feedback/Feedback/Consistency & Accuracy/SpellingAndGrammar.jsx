...
function SpellingAndGrammar({ speed = 10 }) {
  const [target, setTarget] = useState(0);

useEffect(() => {
  const SpellingAndGrammarScore = async () => {
    try {
  ...
  SpellingAndGrammarScore();
}, []);

  const { progress, getProgressColor } = useSpellingAndGrammarFunction(target, speed);
